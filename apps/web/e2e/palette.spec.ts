...
test("empty query: a short root list that does NOT dump every surface into it", async ({ page }) => {
  ...
  // would be satisfied by "Settings: Fleet" and stop testing the quota at all. It is also
  // the platform-independent half — `formatCombo` renders ⌘ on macOS and Ctrl on CI.
  await expect(
