...
export default defineConfig({
  test: {
    ...
    // `.tsx` too: a component-shaped test (FleetRoom.test.tsx, #3169) renders presentational
    // pieces via react-dom/server, so it needs the JSX extension the pure-logic `.test.ts`
    // suite never did.
