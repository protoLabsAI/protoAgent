...
function TelemetryBody() {
  ...
  const tabItems = telemetryTabItems(Boolean(fleet.fleet));
  const [tab, setTab] = useState<TelemetryTabId>(() => defaultTelemetryTab(hasTurns, Boolean(fleet.fleet)));
  // A tab that stops existing (the fleet went away between refreshes) must not leave
  ...
  return (
    <>
      ...
      <div className="stage-body">
        ...
        ) : !hasTelemetryViews(summary, fleet) ? (
          <Empty>No turns recorded yet — run a turn and refresh.</Empty>
        ) : (
          <>
            {/* The pinned headline: what the surface is opened FOR. Absent when this
                box has no turns of its own — a fleet hub still gets the Fleet tab. */}
            {hasTurns && insights ? (
            ...
            {active === "fleet" ? (
              ...
              summary && summary.by_model.length > 0 ? (
                ...
                <Empty>No per-model rollup yet.</Empty>
              )
            ) : active === "tools" ? (
