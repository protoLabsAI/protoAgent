- **The gateway client no longer lets a model's NAME choose its wire protocol (#3424).**
  ...
  them are guaranteed to serve. It left the choice to langchain-openai, which as of 1.6.2
  infers it from the model name: any name merely *containing* `codex`, plus a handful of
  `gpt-5.x-pro` / `gpt-5.6-sol` prefixes, silently switch to `/v1/responses`. But a
  gateway alias like `protolabs/codex` is a name *we* choose and says nothing about the
  endpoint behind it, so the request went to a path a local runtime does not serve. The
