- **A2A delegate calls now trust what the OS trusts, not just certifi (#2643).**
  httpx always verified against the bundled `certifi` root list alone. A private
  CA an operator installed in the OS certificate store (Windows cert store, macOS
  Keychain, a Linux distro bundle) was invisible to it even though the OS's own
  HTTP clients trusted it fine —
