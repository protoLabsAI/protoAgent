- **The console's keyboard actions are ⌘K commands now, each showing the chord it is bound to
  ...
  **Previous chat tab**, **Toggle latest tool block**, and the left rail / right panel / bottom
  ...
  when an operator would reach for them. A chat-scoped action navigates to chat *before* it
  runs, so choosing **Clear conversation** from Knowledge does what you meant rather than firing
  at a surface nobody is on.
