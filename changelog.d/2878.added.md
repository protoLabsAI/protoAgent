- **CI renders the desktop app now, and the marketing site builds on PRs (#2878).** Two blind
  ...
  crash that lands *after* first paint. Wired into `desktop-build.yml`'s Linux leg. It must not
  be run under `xvfb-run`: that fails 100% of the time for reasons unrelated to the build.
