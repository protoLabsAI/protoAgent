- **The chat's slash commands and your user-facing skills are now in the command palette
  ...
  find any of them. They are the console's real verbs, so they now list in ⌘⇧K too, searchable
  by the words you would actually reach for ("wipe" finds `/clear`, "llm" finds `/model`,
  "latency" finds `/perf`). Enabling a plugin or authoring a skill adds its row immediately —
  the list is read live, not snapshotted at startup — and `/new` and `/clear` show the keyboard
  shortcut you have them bound to rather than a hardcoded one.
  ...
  **A skill row hands you a draft rather than pretending to run.** A user-facing skill isn't
  ...
  mistaken for a fire-and-forget action. `/btw` behaves the same way, since it needs the
