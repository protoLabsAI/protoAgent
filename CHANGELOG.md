...
### Added
- **`protoagent fleet` talks to the running hub (#3467).** `fleet ls|up|down` used to read `fleet.json` from whatever instance root the shell happened to resolve — beside the desktop hub that was a fabricated fleet of one, with the CLI's own pid as the "running" host. They now find a hub by evidence (`server.pid`, the `.instances/` heartbeats across every box root incl. the desktop app's, then `:7870`), open it with its fleet service token, and show `live · <url>`; disk is a badged fallback (`offline`), `--json` is on every verb, and `--hub URL` / `--token` / `PROTOAGENT_HUB_TOKEN` reach a hub elsewhere. In live mode `up`/`down` go through the hub's control plane so it keeps owning its members. First slice of the fleet deck (epic #3466).
...
- **Every hub on the box, from the deck and the CLI (#3472).** `H` in the deck (or `protoagent fleet --all`, with `--json`) lists the hubs this machine runs — the desktop app's, `~/.protoagent`, each scoped instance — and peers found on this box's ports and the tailnet (the LAN too when `fleet.discovery.mdns` is on): state, launcher (desktop app / `protoagent up` / foreground), port, version, member counts, instance root. Running hubs come from the heartbeats under every known box root and stopped ones from every instance root with a fleet, this shell's own instance is one input among them, never the only one; a hub that answers but refuses every credential reads `unauthorized`, one that does not answer `unreachable`. `enter` attaches the deck to a hub (its own fleet token), `u` brings a stopped hub up (`protoagent up` for that instance root) and attaches when its port answers; two hubs claiming one port both say so. Sixth slice of the fleet deck (epic #3466).
