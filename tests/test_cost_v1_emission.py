"""cost-v1 extension: cache fields + costUsd emission (ADR 0006 Slice 1, A2A 1.0).

The terminal artifact carries the Anthropic-shaped cache fields and a top-level
costUsd. Since protolabs-a2a 0.3.0 the payload rides the artifact's **metadata map
keyed by the cost-v1 extension URI** (``pa.cost_metadata``) rather than a MIME-typed
DataPart; the executor accumulates usage across LLM calls and passes it in. Also
verifies ``metrics.record_llm_call`` accepts the enriched signature without a live
Prometheus registry.
"""
...
def test_cost_payload_omits_costusd_when_not_supplied() -> None:
    meta = pa.cost_metadata({"input_tokens": 10, "output_tokens": 5}, duration_ms=15)
    payload = pa.parse_cost(meta)
    assert payload is not None
...
def test_extension_uri_is_the_canonical_cost_uri() -> None:
    ...
    # interceptor on — frozen so we don't silently break the wire contract. Since
    # 0.3.0 this URI is also the METADATA KEY the payload rides under (the MIME
    # constant that used to discriminate the DataPart is gone).
    assert pa.COST_EXT_URI == "https://proto-labs.ai/a2a/ext/cost-v1"
    assert set(pa.cost_metadata({"input_tokens": 1, "output_tokens": 1})) == {pa.COST_EXT_URI}
