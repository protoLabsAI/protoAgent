...
def _tools(monkeypatch, tmp_path):
    """The three watch tools, wired to a throwaway store with one plugin verifier known."""
    ...
    create, listing, clear = _build_watch_tools()
    return ctrl, create, listing, clear
...
def test_create_watch_without_knobs_is_unbounded(monkeypatch, tmp_path):
    ...
    ctrl, create, _l, _c = _tools(monkeypatch, tmp_path)
...
def test_status_line_shows_the_lifetime_knobs_it_was_given(monkeypatch, tmp_path):
    ctrl, create, listing, _c = _tools(monkeypatch, tmp_path)
    create.invoke(
        {
