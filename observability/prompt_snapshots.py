...
class PromptSnapshotStore:
        ...
        try:
            ...
            if self.max_calls > 0:
                ...
                trimmed += db.execute(
                    ...
                    " SELECT id FROM calls ORDER BY id DESC LIMIT -1 OFFSET ?)",
                    (self.max_calls,),
                ).rowcount
            if trimmed:
    ...
    def _sweep_orphan_blobs(db: sqlite3.Connection) -> None:
        ...
        db.execute(
            "DELETE FROM stable_blobs WHERE hash NOT IN (SELECT DISTINCT stable_hash FROM calls)"
        )
    ...
    def last_for_session(self, session_id: str = "") -> dict | None:
        ...
        try:
            row = db.execute(
                f"{self._CALL_SELECT} {where} ORDER BY c.id DESC LIMIT 1", params
            ).fetchone()
        except sqlite3.DatabaseError as exc:
