...
> **Correction (2026-07-20).** Two claims in this paragraph did not survive
> implementation. `hitl-mode-v1` is **not** declared on the card and does not
> exist anywhere in the tree — HITL is discovered from the `hitl-v1` DataPart's
> mimeType on the `INPUT_REQUIRED` frame, not from a card capability. And the
> `blast-v1` / `effect-domain-v1` commented stanza is gone from
> `server/a2a.py::_build_agent_card_proto`.
>
> Nor did the extension list survive. `confidence-v1` was declared but never
> emitted — its `<output>`-protocol producer was deleted by the parser
> retirement, and the card declaration was never cleaned up with it — so it has
> since been **dropped from the card**. The card no longer defaults to
> `protolabs_a2a.ALL_EXTENSION_URIS` (the fleet-wide *vocabulary*) but declares
> the three extensions with a live emission path, via
> `server/a2a.py::_emitted_extension_uris`: `cost-v1`, `worldstate-delta-v1`,
> `tool-call-v1`.
