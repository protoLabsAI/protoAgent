...
| `cowork` | *(no tools — ships the knowledge-work skill pack: `docx` / `xlsx` / `pptx` / `pdf` deliverables through `execute_code`, `/daily-brief`, `drop-folder` watches via the `cowork:folder_changed` verifier, `schedule`, `consolidate-memory`, `writing-voice`, `/setup-cowork`. [ADR 0083](/adr/0083-cowork-mode-archetype))* | off — `plugins.enabled: [cowork]` |
