...
Two things write entries here besides you. A bundle's Configure step: a `config_inputs` `path` prompt flagged `project: true` registers the answered checkout at create time (core ≥ 0.146, #2977) — `{name: <dir name>, path, github: <owner/name from its origin remote>, write: false}` — and, **only when no `onboarding:` section exists**, seeds `onboarding: {enabled: true, root: <the checkout's parent>}` so the agent can `onboard_project` within exactly that tree. And the `onboard_project` tool itself (when `onboarding` below allows it). `show_config(section="projects")` shows what the running agent resolved.
