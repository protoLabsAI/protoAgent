...
| `host` | `http://host.docker.internal:3001` | Base URL of the Langfuse instance **for the key pair below**. The default only resolves inside the bundled Docker compose — a desktop-launched agent needs a real URL. Env-supplied credentials use `LANGFUSE_HOST` / `LANGFUSE_URL` and never this (#3039). |
