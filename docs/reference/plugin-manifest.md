...
Console surfaces ([ADR 0026](/adr/0026-plugin-contributed-console-surfaces)) — each entry adds a left-rail icon opening a full view (an iframe of a page the plugin serves at `path`). Declared as data so it's known without importing the plugin, and surfaced to the frontend via /api/runtime/status. Each: {id, label, icon, path, tabs?, slot?}. `path` must (1) be a path a registered router actually serves — the console iframes it verbatim, so a path no router answers is a blank surface — and (2) be a same-origin RELATIVE path (no scheme/host/port): an absolute URL escapes the [ADR 0042](/adr/0042-fleet-supervisor-unified-console) fleet proxy origin and breaks the same-origin postMessage token handshake. See `_parse_views` (warns on non-same-origin) and docs/guides/building-react-plugin-views.md.
