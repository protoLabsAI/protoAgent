...
| `LANGFUSE_HOST` | Langfuse host URL (e.g. `https://langfuse.company.com`). Falls back to `LANGFUSE_URL`, then the `tracing.host` config key, then `http://host.docker.internal:3001`. |
