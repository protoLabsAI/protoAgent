...
| `LANGFUSE_HOST` | Langfuse host URL (e.g. `https://langfuse.company.com`) **for env-supplied keys**. Falls back to `LANGFUSE_URL`, then `http://host.docker.internal:3001` — never to the `tracing.host` config key (#3039). |
