...
| [#225](https://github.com/protoLabsAI/protoContent/issues/225) | `SideNav` — vertical section navigation (`Tabs` is horizontal-only; the settings Workspace home's 11 sections overflowed/read as "intense" in a strip) | P2 | Filed 2026-06-12. **App-side interim shipped**: `SettingsSurface` renders a vertical rail (scope toggle pinned on top + `.settings-sidenav-list` section buttons), marked with this #. Collapsible field groups needed no DS work — adopted the existing `Accordion`/`AccordionItem` (0.29). Retire the interim when `SideNav` lands. |
