...
| _(unfiled — see Context)_ | **`SortableList` / a reorder seam with a wired KEYBOARD path** — a list whose rows are drag-reorderable AND arrow-key-reorderable from a focus handle | P2 | **Confirmed gap in ui 0.60.1** (2026-09-01). The DS *bundles* `@dnd-kit/core`+`sortable`, but exposes them only as two closed surfaces: `AppShell`'s `onRailReorder` rails (rail-shaped; a `KeyboardSensor` IS wired there) and `TabBar` (`PointerSensor` only — its keyboard path is explicitly unwired). Neither takes arbitrary row content, so a rich fleet row can't use them. **Evidence:** `apps/web/src/settings/FleetManagerPanel.tsx` (#3197) reorders the fleet roster with native HTML5 DnD + an `onKeyDown` ↑/↓ handler inline — no new dependency and no bespoke *reusable* component, so the "file before hand-rolling" gate isn't crossed by a one-off. **Not filed as its own issue yet:** one consumer. File it on protoContent the moment a second list wants reorder (the workflow builder's steps are the likely next), and hand over this implementation. |
